...
(describe "edge cases"
  ...
  (describe "log-level macros"
    ;; LOG-<LEVEL> and LOG-DEFAULT-<LEVEL> are macros generated by
    ;; DEFINE-LOG-LEVEL-MACROS: a macro name cannot be handed to IT-EACH as
    ;; data and invoked through a runtime variable, so each of the ten
    ;; generated macros is called by its literal name here instead.
    (it "log-<level> passes its literal level through for every rank"
      (multiple-value-bind (logger fetch) (capturing-logger :level +level-debug+)
        (log-debug logger "d") (expect (funcall fetch) :to-contain-substring "level=DEBUG")
        (log-info  logger "i") (expect (funcall fetch) :to-contain-substring "level=INFO")
        (log-warn  logger "w") (expect (funcall fetch) :to-contain-substring "level=WARN")
        (log-error logger "e") (expect (funcall fetch) :to-contain-substring "level=ERROR")
        (log-fatal logger "f") (expect (funcall fetch) :to-contain-substring "level=FATAL")))

    (it "log-default-<level> routes through *default-logger* for every rank"
      (multiple-value-bind (logger fetch) (capturing-logger :level +level-debug+)
        (let ((*default-logger* logger))
          (log-default-debug "d") (expect (funcall fetch) :to-contain-substring "level=DEBUG")
          (log-default-info  "i") (expect (funcall fetch) :to-contain-substring "level=INFO")
          (log-default-warn  "w") (expect (funcall fetch) :to-contain-substring "level=WARN")
          (log-default-error "e") (expect (funcall fetch) :to-contain-substring "level=ERROR")
          (log-default-fatal "f") (expect (funcall fetch) :to-contain-substring "level=FATAL")))))
        ...
        (setf multi (make-multi-handler
                      (list (make-function-handler
                              (lambda (record)
                                ...
                                (let ((condition (handler-case (close-handler multi)
                                                   (handler-lifecycle-error (c) c))))
                                  (expect (typep condition 'handler-lifecycle-error) :to-be-truthy)
                                  (expect (eq (handler-lifecycle-error-handler condition) multi)
                                          :to-be-truthy)
                                  (expect (eq (handler-lifecycle-error-operation condition) :close)
                                          :to-be-truthy)))))))
